...
Kras_mt_Right<- read.csv("second_meta-analysis/AD KM-plots/Chen/Right_Side_Mutated.csv")
...
Kras_mt_Left<- read.csv("second_meta-analysis/AD KM-plots/Chen/Left_Side_Mutated.csv")
...
### Mutated Rectum Side (68)


tot.events<-"NA" #tot.events = total no. of events reported. If not reported, then tot.events="NA"
arm.id<-"Kras-mt_Rectum" #arm indicator
###END FUNCTION INPUTS
#Read in survival times read by Kras_wt_Rectum
Kras_mt_Rectum<- read.csv("second_meta-analysis/AD KM-plots/Chen/Rectum_Side_Mutated.csv")

t.S<-Kras_mt_Rectum[,1]
S<-Kras_mt_Rectum[,2]
...
###END FUNCTION INPUTS
#Read in survival times read by Kras_wt_Left
Kras_wt_Right<- read.csv("second_meta-analysis/AD KM-plots/Chen/Right_Side_Wild_Type.csv")
...
###END FUNCTION INPUTS
#Read in survival times read by Kras_wt_Left
Kras_mt_Left<- read.csv("second_meta-analysis/AD KM-plots/Chen/Left_Side_Wild_Type.csv")
...
IPD= rbind(IPD, temp)
############ Wild type Rectum side 


rm(list=ls()[! ls() %in% c("IPD")])
tot.events<-"NA" #tot.events = total no. of events reported. If not reported, then tot.events="NA"
arm.id<-"Kras-wt_Rectum" #arm indicator
###END FUNCTION INPUTS
#Read in survival times read by Kras_wt_Rectum
Kras_mt_Rectum<- read.csv("second_meta-analysis/AD KM-plots/Chen/Rectum_Side_Wild_Type.csv")
...
plot(test.surv, col = c(1,2,3,4));

#### Save file


write.csv(x = IPD,"second_meta-analysis/AD KM-plots/Chen/Simulated_data.csv")
